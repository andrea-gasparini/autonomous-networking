...
class HW2Routing(BASE_routing):
	...
	def feedback(self, drone: Drone, id_event: int, delay: int, outcome: int, depot_index=None) -> None:
	...
	def __exploration(self, neighbors_drones: Set[Drone]) -> Union[None, Literal[-1], Drone]:
		...
		if len(neighbors_drones) == 0:
			...
			action = self.best_depot(self.drone.coords) if len(neighbors_returning_to_depot) == 0 else self.rnd_for_routing_ai.choice(list(neighbors_returning_to_depot))
		else:
			action = None
	...
	def __exploitation(self, neighbors_drones: Set[Drone], cell_index: int) -> Union[None, Literal[-1], Drone]:
		...
		if first_depot_distance <= 400 and first_depot_distance < second_depot_distance:
			return -1
		elif second_depot_distance <= 400:
			return -2
		
		if len(neighbors_drones) == 0 and self.drone.buffer_length() >= 2:
			return self.best_depot(self.drone.coords)
	...
	def relay_selection(self, opt_neighbors: List[Tuple[DataPacket, Drone]], pkd: DataPacket) -> Union[None, Literal[-1], Drone]:
		...
		if cell_index not in self.q_table:
			self.q_table[cell_index] = [0 for i in range(self.simulator.n_drones + 2)]
		...
		if next_cell_index not in self.q_table:
			self.q_table[next_cell_index] = [0 for i in range(self.simulator.n_drones + 2)]
