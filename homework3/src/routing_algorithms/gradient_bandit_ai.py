...
class GradientBanditAI(BASE_routing):
    ...
    def feedback(self, drone: Drone, id_event, delay, outcome, depot_index = None):
        ...
        if id_event in self.taken_actions:
            ...
            if outcome == -1: 
                ...
                reward = -2
                self.update_H(action, reward, actions, mean_reward, prob_action)
    ...
    def compute_lr(self) -> float:
        return self.alpha * np.exp(-self.decay * self.simulator.cur_step)
    
    def update_H(self, action, reward, actions, mean_reward, prob_action):
        learning_rate = self.compute_lr()
        if action == -1 or action == -2:
            self.H[action] += learning_rate * (reward - mean_reward) * (1 - prob_action[action])
        else:
            self.H[action.identifier] += learning_rate * (reward - mean_reward) * (1 - prob_action[actions.index(action)])
        ...
        for i, act in enumerate(actions):
            ...
            if act != -1 and act != -2:
                self.H[act.identifier] -= learning_rate * (reward - mean_reward) * (prob_action[i])
            else:
                self.H[act] -= learning_rate * (reward - mean_reward) * (prob_action[-1])
    ...
    def relay_selection(self, opt_neighbors, pkd):
        ...
        if action != -1 and action != -2:
            ...
            avg_packets = 1
            if action.buffer_length() != 0:
                for pkt in action.all_packets():
                    avg_packets += self.simulator.cur_step - pkt.time_step_creation
                avg_packets = avg_packets / action.buffer_length()

            reward = action.speed * action.buffer_length() / (distance * avg_packets)
        else:
            ...
            reward = self.drone.speed * self.drone.buffer_length() / (distance * math.log(energy_spent))

        self.n += 1
        self.mean_reward += (reward - self.mean_reward) / self.n

        self.update_H(action, reward, actions, self.mean_reward, self.prob_action)
